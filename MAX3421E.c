...
#include "stdio.h"
#include "string.h"
#include "../finalproject/project_config.h"
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wdata[0] = reg + 2;
	wdata[1] = val;
	returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 2, wdata, 0,pointer,0);
	if(returncode < 0){
		printf("error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
		int i;
		alt_u8* pointer_2;
		alt_u8 answer[nbytes + 1];
		answer[0] = reg + 2;
		for(i = 0; i < nbytes ; i++){
			answer[i+1] = data[i];
		}
		//psuedocode:
		//select MAX3421E (may not be necessary if you are using SPI peripheral) CHECK
		//write reg + 2 via SPI
		int returncode = alt_avalon_spi_command(SPI_0_BASE,0,nbytes + 1,answer,0,pointer_2,0);
		// where write data[n] via SPI,e n goes from 0 to nbytes-1 CHECK
		//read return code from SPI peripheral (see Intel documentation)
		//if return code < 0  print an error
		if(returncode < 0){
			printf("error");
		}
		//deselect MAX3421E (may not be necessary if you are using SPI peripheral) CHECK
		//return (data + nbytes);
		return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
		...
		alt_u8 rdata[1];
		//read val via SPI
		returncode = alt_avalon_spi_command(SPI_0_BASE,0, 1, wdata, 1,rdata,0 );
		//read return code from SPI peripheral (see Intel documentation)
		//if return code < 0 print an error
		if(returncode < 0){
			printf("error");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
		...
		int i;
		alt_u8 wdata[1] = {reg};
		returncode = alt_avalon_spi_command(SPI_0_BASE,0, 1, wdata, nbytes,data,0 );
		if(returncode < 0){
			printf("error");
		}
		//psuedocode:
